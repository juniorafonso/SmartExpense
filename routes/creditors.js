...
module.exports = (db, authMiddleware) => {
  ...
  router.get('/', requireLogin, (req, res) => {
    ...
    db.all('SELECT * FROM creditors ORDER BY name ASC', (err, creditors) => {
      if (err) {
        ...
        req.flash('error', res.locals.lang.error_fetching_creditors || 'Error fetching creditors.');
        return res.render('creditors', { creditors: [] }); // Render page even on error
      }
      // Assuming 'creditors.ejs' exists and expects 'creditors' array and flash messages
      res.render('creditors', { creditors: creditors || [] });
    });
  });
  ...
  router.post('/', requireLogin, (req, res) => {
    ...
    if (!creditorName) {
      req.flash('error', res.locals.lang.error_creditor_name_required || 'Creditor name is required.');
    ...
    db.run('INSERT INTO creditors (name, information) VALUES (?, ?)', [creditorName, information || null], function(err) {
      if (err) {
        ...
        if (err.message.includes('UNIQUE constraint failed')) {
             ...
             req.flash('error', res.locals.lang.error_creditor_name_taken || `Creditor name '${creditorName}' is already taken.`);
        } else {
             req.flash('error', res.locals.lang.error_adding_creditor || 'Error adding creditor.');
        }
      } else {
        console.log(`[POST /creditors] Creditor '${creditorName}' (ID: ${this.lastID}) added.`);
        req.flash('success', res.locals.lang.success_creditor_added || `Creditor '${creditorName}' added successfully.`);
      }
      ...
      if (err) {
        console.error(`[POST /creditors/${creditorId}/edit] Error editing creditor:`, err.message);
         if (err.message.includes('UNIQUE constraint failed')) {
            ...
            req.flash('error', res.locals.lang.error_editing_creditor || 'Error editing creditor.');
        }
      } else if (this.changes > 0) {
        console.log(`[POST /creditors/${creditorId}/edit] Creditor updated to '${creditorName}'.`);
        req.flash('success', res.locals.lang.success_creditor_updated || `Creditor '${creditorName}' updated successfully.`);
      } else {
        console.warn(`[POST /creditors/${creditorId}/edit] No rows affected. Creditor ID ${creditorId} may not exist.`);
        req.flash('warning', res.locals.lang.warning_creditor_not_found || 'Creditor not found or no changes made.');
      }
      res.redirect('/creditors');
    });
  });
  ...
  router.post('/:id/delete', requireLogin, /* requireAdmin, */ (req, res) => {
    ...
    db.run('DELETE FROM creditors WHERE id = ?', [creditorId], function(err) {
      if (err) {
        ...
        req.flash('error', res.locals.lang.error_deleting_creditor || 'Error deleting creditor.');
      } else if (this.changes > 0) {
        console.log(`[POST /creditors/${creditorId}/delete] Creditor deleted successfully.`);
        req.flash('success', res.locals.lang.success_creditor_deleted || 'Creditor deleted successfully.');
      } else {
        console.warn(`[POST /creditors/${creditorId}/delete] No rows affected. Creditor ID ${creditorId} may not exist.`);
        req.flash('warning', res.locals.lang.warning_creditor_not_found_delete || 'Creditor not found.');
      }
      res.redirect('/creditors');
    });
  });
